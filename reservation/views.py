...
from .models import Bus, Book, Profile
...
def viewprofile(request):
    ...
    # Create a dictionary with user data for the template
    dic = {
        'id': user_r.id,
        'username': user_r.username,
        'email': user_r.email,
        'phno': phno_r,
        'first_name': user_r.first_name,
        'last_name': user_r.last_name,
    }
...
def bookings(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            context["error"] = "Sorry, this bus is no longer available. Please search again."
        ...
        if bus:
            if bus.rem >= seats_r:
                ...
                context["error"] = "Sorry, only " + str(bus.rem) + " seats available. Please select fewer seats."
                return render(request, 'findbus.html', context)
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if user:
            userprofile = Profile.objects.create(user=user, phno=phno_r)
